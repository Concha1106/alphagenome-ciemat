...
def export_splice_junctions(prediction, output_dir):
    ...
    for track_index in range(values_ref.shape[1]):
        ...
        for junction_index in range(values_ref.shape[0]):
            ...
            if hasattr(junctions, "iloc"):
                row.update(junctions.iloc[junction_index].to_dict())
            else:
                row["junction"] = junctions[junction_index]
...
def write_runlog(args, output_dir, variant, interval, requested_outputs, ontology_terms):
    ...
    with open(runlog_path, "w", encoding="utf-8") as runlog:
        ...
        runlog.write("predict_variant_splice_junctions.tsv\n")
